...
public:
	...
	void savePC(bool setting_savepcdfileRequested );
	void saveOctreeMap ( bool setting_saveOctoMapRequested);

	// copies & filters internal data to GL buffer for rendering. if nothing to do: does nothing.
	bool refreshPC(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity, std::vector<Vec3f,Eigen::aligned_allocator<Vec3f>> allFramePoses_to_searchPoints);

	// renders cam & pointcloud.
	void drawCam(float lineWidth = 1, float* color = 0, float sizeFactor=1);
	void drawPC(float pointSize);
	void drawMap(float pointSize);
	void drawObstacle(float pointSize);

	void quickSort( float tmpPoint[], int low , int high);

	int id;
	bool active;
	SE3 camToWorld;
	std::vector<Vec3f,Eigen::aligned_allocator<Vec3f>> allFramePoses_to_searchPoints;
...
private:
	...
	bool bufferValid;
	bool mapbufferValid = false;
	// bool obstacleValid = false;
	int numGLBufferPoints;
	int numGLBufferGoodPoints;
	int numMapGLBufferPoints;
	int numMapGLBufferGoodPoints;
	int numObstaclePoints;
	pangolin::GlBuffer vertexBuffer;
	pangolin::GlBuffer colorBuffer;
	pangolin::GlBuffer mapvertexBuffer;
	pangolin::GlBuffer mapcolorBuffer;
	// pangolin::GlBuffer obstacleBuffer;
