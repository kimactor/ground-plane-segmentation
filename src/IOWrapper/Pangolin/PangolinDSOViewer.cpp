...
void PangolinDSOViewer::run()
{
	...
	// 3D visualization
	pangolin::OpenGlMatrix proj = pangolin::ProjectionMatrix(w,h,400,400,w/2,h/2,0.1,1000);
	pangolin::OpenGlRenderState Visualization3D_camera(proj, pangolin::ModelViewLookAt(-0,-5,-10, 0,0,0, pangolin::AxisNegY ) );
	pangolin::OpenGlRenderState Visualization3D_map(proj, pangolin::ModelViewLookAt(0, -10, -0.5, 0, 0, 0, pangolin::AxisNegY ) );
	...
	pangolin::View& d_map = pangolin::Display("Visualization3D_map")
		.SetAspect(w/(float)h)
		.SetHandler(new pangolin::Handler3D(Visualization3D_map)
		);
	...
	pangolin::GlTexture texMap(w,h,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	...
	pangolin::Var<bool> settings_showLiveMap("ui.showMap",false,true);
	...
	pangolin::Var<bool> settings_savePCDButton("ui.savePCDFile",false,false);
	pangolin::Var<bool> settings_saveOctreeButton("ui.saveOctreeFile",false,false);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		if(setting_render_display3D)
		{
			...
			for(KeyFrameDisplay* fh : keyframes)
			{
				...
				refreshed =+ (int)(fh->refreshPC(refreshed < 10, this->settings_scaledVarTH, this->settings_absVarTH,
						this->settings_pointCloudMode, this->settings_minRelBS, this->settings_sparsity, this->allFramePoses_to_searchPoints));
				fh->drawPC(1);
				// fh->drawObstacle(1);
			}
		...
		if(setting_render_displayMap)
		{
			d_map.Activate(Visualization3D_map);
			boost::unique_lock<boost::mutex> lk3d(model3DMutex);
			//pangolin::glDrawColouredCube();
			int refreshed=0;
			for(KeyFrameDisplay* fh : keyframes)
		...
		setting_render_displayMap = settings_showLiveMap.Get();
		...
		if ( settings_savePCDButton.Get() )
		{
			setting_savepcdfileRequested = true;
			KeyFrameDisplay kfd;
			kfd.savePC( setting_savepcdfileRequested );
			settings_savePCDButton.Reset();	
		}
		if ( settings_saveOctreeButton.Get() )
		{
			setting_saveOctoMapRequested = true;
			KeyFrameDisplay kfd;
			kfd.saveOctreeMap( setting_saveOctoMapRequested );
			settings_saveOctreeButton.Reset();	
		...
		CalibHessian* HCalib)
{
	...
	allFramePoses_to_searchPoints.push_back(frame->camToWorld.translation().cast<float>());
}
