...
//--------------------------------define tree------------------// 
float accuracy = 0.02;
octomap::ColorOcTree tree( accuracy ); //全局map
int number_of_frames = 0;
// octomap::OcTree tree( 0.02 ); //全局map
...
bool KeyFrameDisplay::refreshPC(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity)
{
	...
	for(int i=0;i<numSparsePoints;i++)
	{
		...
		for(int pnt=0;pnt<patternNum;pnt++)
		{
			...
			// pp.x= tmpVertexBuffer[vertexBufferNumPoints][0]; 
			// pp.y = tmpVertexBuffer[vertexBufferNumPoints][1]; 
			// pp.z = tmpVertexBuffer[vertexBufferNumPoints][2];
...
//modify part
	// printf("----------------------------dealing with octomap!-------------------------\n");
//------------------------------deal with octomap and pcl----------------//

	
	pcl::PointCloud<pcl::PointXYZRGBA>::Ptr temp(new pcl::PointCloud<pcl::PointXYZRGBA>());
	pcl::transformPointCloud(cloud, *temp, camToWorld.matrix().cast<float>());

	Sophus::Matrix4f mf = camToWorld.matrix().cast<float>();
	// Sophus::Vector3d tr = camToWorld.translation();

	for(auto p:temp->points){
		// if(p.y <= mf(1,3)+2*accuracy && p.y>= mf(1,3)-15*accuracy)
		// {
		// 	p.y=0;
	...
	// if(number_of_frames % 1000 == 0)
	if(number_of_frames >= 4500 )
	{
		tree.updateInnerOccupancy();
		tree.write("/home/kim/dso/full_octomap/pioneer.bt"); 
		// tree.writeBinary("/home/kim/dso/full_octomap/pioneer.bt"); 

		// pcl::io::savePCDFileBinary("/home/kim/dso/pcl_map/pioneer.pcd",cloud_pcl);
		// pcl_writer.write("/home/kim/dso/pcl_map/pioneer.pcd",*temp);
	}
	number_of_frames +=1;
